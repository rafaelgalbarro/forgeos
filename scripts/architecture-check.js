 ...
 *   node scripts/architecture-check.js --strict   # promote selected WARNs to fail
 */
const fs = require("fs");
...
function checkDomainPurity() {
  ...
  const reactRe = /from\s+["']react(?:\/[^"']*)?["']|from\s+["']next(?:\/[^"']*)?["']|require\(\s*["']react|require\(\s*["']next/;
  const storageRe = /\blocalStorage\b|\bsessionStorage\b/;
  for (const file of collectSourceFiles(domainRoot)) {
    const rel = toPosix(path.relative(ROOT, file));
    const src = stripComments(read(file));
    if (reactRe.test(src)) {
      addCrit(`React/Next in core domain: ${rel}`);
    }
    if (storageRe.test(src)) {
      addCrit(`Web storage API in core domain: ${rel}`);
    }
  }
}
...
function checkKnownCircular() {
  ...
  const mcSrc = read(mcIndex);
  const mcImportsLive = /@\/lib\/live-mission/.test(mcSrc);
...
function main() {
  ...
  log("  Mode: warn-friendly (exit 0 unless CRITICAL). Use --strict to fail on soft presentation/capability hits.");
