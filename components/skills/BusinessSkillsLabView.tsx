...
import {
  runBusinessSkillsLab,
  type BusinessSkillsLabSnapshot,
} from "@/lib/lab/business-skills-lab";
...
export function BusinessSkillsLabView() {
  const [data, setData] = useState<BusinessSkillsLabSnapshot | null>(null);

  useEffect(() => {
    runBusinessSkillsLab().then(setData);
  }, []);

  if (!data) {
    return (
      <Container>
        <p>Cargando Business Skills…</p>
      </Container>
    );
  }

  const paymentsSample = data.sampleExecutions["business-payments"];
  const contractsSample = data.sampleExecutions["business-contracts"];

  return (
    <Container className="fhis-business-skills-lab">
      <SectionHeader
        title="Business Skills"
        subtitle="RC4.4 — CRM, ERP, Accounting, Payments & more (sandbox mock)"
      />

      <Stack gap="lg">
        <Panel>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
            <KpiBlock label="Business Skills" value={String(data.health.total)} />
        ...
        <Panel>
          <SectionHeader title="Domains" subtitle="8 enterprise provider modules" />
        ...
        {(paymentsSample || contractsSample) && (
          <Panel>
            <SectionHeader title="High-Risk Samples" subtitle="Payments & Contracts — governed" />
            {paymentsSample?.skillResult && (
              <p style={{ fontSize: 13, marginBottom: 8 }}>
                Payments: {paymentsSample.skillResult.output}{" "}
                <Badge variant={riskVariant(paymentsSample.risk.level)}>
                  {paymentsSample.risk.level}
                </Badge>
              </p>
            )}
            {contractsSample?.skillResult && (
              <p style={{ fontSize: 13 }}>
                Contracts: {contractsSample.skillResult.output}{" "}
                <Badge variant={riskVariant(contractsSample.risk.level)}>
                  {contractsSample.risk.level}
                </Badge>
              </p>
            )}
          </Panel>
        ...
        <Panel>
          <SectionHeader title="Telemetry" subtitle={`${data.telemetry.length} business records`} />
