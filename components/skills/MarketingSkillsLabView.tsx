...
import {
  runMarketingSkillsLab,
  type MarketingSkillsLabSnapshot,
} from "@/lib/lab/marketing-skills-lab";
...
export function MarketingSkillsLabView() {
  const [data, setData] = useState<MarketingSkillsLabSnapshot | null>(null);

  useEffect(() => {
    runMarketingSkillsLab().then(setData);
  }, []);

  if (!data) {
    return (
      <Container>
        <p>Cargando Marketing Skills…</p>
      </Container>
    );
  }

  const adsSample = data.sampleExecutions["marketing-ads"];
  const emailSample = data.sampleExecutions["marketing-email"];
  const campaignsSample = data.sampleExecutions["marketing-campaigns"];

  return (
    <Container className="fhis-marketing-skills-lab">
      <SectionHeader
        title="Marketing Skills"
        subtitle="RC4.5 — Campaigns, SEO, Ads, Email & more (sandbox mock)"
      />

      <Stack gap="lg">
        <Panel>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
            <KpiBlock label="Marketing Skills" value={String(data.health.total)} />
        ...
        <Panel>
          <SectionHeader title="Domains" subtitle="8 marketing provider modules" />
        ...
        {(adsSample || emailSample || campaignsSample) && (
          <Panel>
            <SectionHeader title="High-Risk Samples" subtitle="Ads, Email & Campaigns — governed" />
            {adsSample?.skillResult && (
              <p style={{ fontSize: 13, marginBottom: 8 }}>
                Ads: {adsSample.skillResult.output}{" "}
                <Badge variant={riskVariant(adsSample.risk.level)}>{adsSample.risk.level}</Badge>
              </p>
            )}
            {emailSample?.skillResult && (
              <p style={{ fontSize: 13, marginBottom: 8 }}>
                Email: {emailSample.skillResult.output}{" "}
                <Badge variant={riskVariant(emailSample.risk.level)}>{emailSample.risk.level}</Badge>
              </p>
            )}
            {campaignsSample?.skillResult && (
              <p style={{ fontSize: 13 }}>
                Campaigns: {campaignsSample.skillResult.output}{" "}
                <Badge variant={riskVariant(campaignsSample.risk.level)}>
                  {campaignsSample.risk.level}
                </Badge>
              </p>
            )}
          </Panel>
        ...
        <Panel>
          <SectionHeader title="Telemetry" subtitle={`${data.telemetry.length} marketing records`} />
        ...
        <Panel>
          <SectionHeader title="History / Audit Timeline" subtitle="Governance + skill audit" />
