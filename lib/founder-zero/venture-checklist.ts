/** Program 4000 — Automatic checklist per pipeline stage. */
...
import { VALIDATION_PIPELINE } from "./pipeline-stages";
import type { ChecklistStatus, ValidationStage } from "./types";
...
): { status: ChecklistStatus; summary: string; risks: string[]; pending: string[]; recommendations: string[] } {
  ...
  switch (id) {
    ...
    case "naming":
      return venture.name && venture.name !== "Untitled"
        ? { status: "completed", summary: venture.name, risks: [], pending: [], recommendations: [] }
        : { status: "not_started", summary: "Sin nombre", risks: [], pending: ["Naming"], recommendations: ["Venture Factory naming stage"] };
    ...
    case "logo":
      return { status: "in_progress", summary: "Logo pendiente de asset real", risks: [], pending: ["Asset logo"], recommendations: ["Design partner / Capability Layer"] };
    ...
    case "frontend-plan":
      return hasSection(venture, "frontend")
        ? { status: "completed", summary: "Plan frontend listo", risks: [], pending: [], recommendations: [] }
        : { status: "not_started", summary: "Frontend sin planificar", risks: [], pending: ["Stack UI"], recommendations: ["Build Platform frontend"] };

    case "backend-plan":
      return hasSection(venture, "backend")
        ? { status: "completed", summary: "Plan backend listo", risks: [], pending: [], recommendations: [] }
        : { status: "not_started", summary: "Backend sin planificar", risks: [], pending: ["API design"], recommendations: ["Build Platform backend"] };

    case "database-plan":
      return hasSection(venture, "base-datos")
        ? { status: "completed", summary: "Esquema DB definido", risks: [], pending: [], recommendations: [] }
        : { status: "not_started", summary: "DB sin esquema", risks: [], pending: ["Modelo datos"], recommendations: ["Supabase step"] };
    ...
    case "go-to-market":
      return hasSection(venture, "landing") && (hasSection(venture, "kpis") || hasSection(venture, "roadmap"))
        ...
        : hasSection(venture, "landing")
          ? { status: "in_progress", summary: "GTM parcial", risks: [], pending: ["KPIs", "Roadmap"], recommendations: ["Growth department"] }
...
export function buildValidationChecklist(
  ...
  const preliminary = VALIDATION_PIPELINE.map((def) => {
  ...
  const completedRatio = completedCount / VALIDATION_PIPELINE.length;

  return VALIDATION_PIPELINE.map((def) => {
